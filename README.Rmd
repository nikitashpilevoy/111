...
88 Calculate and report the mean and median total number of steps taken per day
```{r, echo=TRUE}
mymean = mean(mydata1)
...
```{r, echo=TRUE}
mydata2 = tapply(data$steps, as.factor(data$interval), mean, na.rm = TRUE)
daily_activity = data.frame(average = mydata2, interval = unique(data$interval), row.names = NULL)
